#! /usr/bin/python
"""
COMPLETE ENUMERATION OF HP PROTEIN CONFORMATIONS - ULTIMATE VERSION (2D)
...
Version: 2.0 (Ultimate Edition - 2D Square Lattice)
Enhancements:
...
Lattice: 2D square lattice with 4 cardinal directions
...
usage = """Usage: python enumerate_ultimate_2D.py <configfile>

Example:  python enumerate_ultimate_2D.py enumerate.conf
...
LATTICE TYPE: 2D square lattice (4 directions)
"""
...
try:
    ...
    from Chain import *         # Protein chain representation on 2D square lattice
...
if __name__ == '__main__':
    ...
    # Warn about computational feasibility for 2D lattice
    if chain_length > 25:
        print("="*70)
        ...
        print("\nEnumeration time scales exponentially (~3^n for 2D lattice)")
        print("Estimated time for 2D square lattice:")
        print("  n=20: ~5 minutes")
        print("  n=25: ~hours")
        print("  n=30: ~days")
    ...
    print(f"Expected scaling: ~3^{chain_length} ≈ {3**chain_length:,.0f} conformations")
    print("="*70)
    ...
    # ------------------------------------------------------------------------
    # 6. ENUMERATION COMPLETE - CALCULATE STATISTICS
    # ------------------------------------------------------------------------
    ...
    try:
        with open(combined_path, "w", encoding="utf-8") as f:
            ...
            f.write("# ===================================================================\n")
            f.write("# 2D SQUARE LATTICE NOTES\n")
            f.write("# ===================================================================\n")
            f.write("# Contact detection: Manhattan distance = 1 in (x,y) plane\n")
            f.write("# Scaling: Conformational space ~ 3^n\n")
            f.write("# Symmetry: 4-fold rotational symmetries eliminated\n")
            f.write("# Move set: Up(0), Right(1), Down(2), Left(3)\n")
            f.write("# ===================================================================\n")
    ...
    # Thermodynamics file
    if thermo:
        thermo_path = os.path.join(outdir, f"{safe_stem}.thermodynamics_2D.tsv")
        print(f"Writing thermodynamics to: {thermo_path}")

        try:
            with open(thermo_path, "w", encoding="utf-8") as f:
                f.write("# ===================================================================\n")
                f.write("# THERMODYNAMIC PROPERTIES - 2D SQUARE LATTICE\n")
                f.write("# ===================================================================\n")
                f.write(f"# HP_SEQUENCE:\t{hpstring}\n")
                f.write(f"# TEMPERATURE:\t{config.T}\n")
                f.write(f"# ENERGY_PER_CONTACT:\t{config.eps}\n")
    ...
    # Ground state file
    if contacts:
        ground_path = os.path.join(outdir, f"{safe_stem}.ground_state_2D.tsv")
        print(f"Writing ground state analysis to: {ground_path}")

        try:
            with open(ground_path, "w", encoding="utf-8") as f:
                f.write("# ===================================================================\n")
                f.write("# GROUND STATE ANALYSIS - 2D SQUARE LATTICE\n")
                f.write("# ===================================================================\n")
                f.write(f"# HP_SEQUENCE:\t{hpstring}\n")
    ...
    print("="*70)
    print("ENUMERATION SUMMARY - 2D SQUARE LATTICE")
    print("="*70)
    ...
    print(f"\nLattice type:               2D square lattice (4 directions)")
    print(f"Computation time:           {format_time(total_time)}")
    print(f"Average rate:               {nconfs/total_time:.1f} conformations/second")
    print(f"\nOutput directory:           {outdir}")
    print(f"Main results file:          {safe_stem}.states_2D.tsv")

    if thermo:
        print(f"Thermodynamics file:        {safe_stem}.thermodynamics_2D.tsv")

    print(f"Ground state file:          {safe_stem}.ground_state_2D.tsv")
    print("="*70)
