#! /usr/bin/python
"""
COMPLETE ENUMERATION OF HP PROTEIN CONFORMATIONS
...
from Monty import *         # Monte Carlo utilities (not used in enumeration)
...
if __name__ == '__main__':
    ...
    # The Chain starts with move vector [0, 0, 0, ..., 0] (all beads at origin)
    ...
    # ------------------------------------------------------------------------
    # 3. COMPLETE ENUMERATION ALGORITHM
    # ------------------------------------------------------------------------
    #
    # This implements a depth-first search through conformational space:
    #   - GROW: Extend chain by placing next bead in available lattice sites
    ...
    # chain on the lattice, subject to constraints:
    #   - Self-avoidance: no two beads occupy same site
    #   - Connectivity: consecutive beads must be adjacent (distance = 1)
    ...
    while not done:
        ...
        if len(replicas[0].chain.vec) == replicas[0].chain.n - 1:
            ...
            # viable = True if:
            #   - No two beads occupy the same lattice site (self-avoidance)
            #   - All beads form a connected path (no breaks)
            #   - All bond lengths equal 1 lattice unit (implicit in moves)
            ...
            if replicas[0].chain.viable:
                ...
                if replicas[0].chain.nonsym():
                    ...
                    #   - |i - j| >= 2 (not bonded neighbors)
                    #   - Beads are adjacent on lattice (distance = 1)
                    # Example: [(0, 9), (3, 7)] means bead 0 touches bead 9,
                    #          and bead 3 touches bead 7
        ...
        # --------------------------------------------------------------------
            ...
            if replicas[0].chain.viable:
                ...
                # grow() adds next bead in one of the 6 lattice directions
                # The method cycles through directions: 0, 1, 2, 3, 4, 5
        ...
        # Due to cubic lattice symmetry, we can explore only half the moves
        # When vec[0] reaches 1, we've completed one "hemisphere" of rotations
        # All remaining conformations would be rotations of already-counted ones
        # This optimization roughly halves the computation time
    ...
    # Calculate and display some useful derived quantities
    print("\n" + "="*70)
